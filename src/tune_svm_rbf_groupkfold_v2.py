...
import tune_svm_rbf_groupkfold as base
...
OUTPUT_DIR = Path("logs") / "svm_rbf_tuning_v2"
...
OOF_CONFUSION_MATRIX_IMAGE_PATH = OUTPUT_DIR / "svm_rbf_confusion_matrix.png"
...
FINAL_TEST_PREDICTIONS_PATH = OUTPUT_DIR / "svm_rbf_final_test_predictions.csv"
FINAL_TEST_CONFUSION_MATRIX_IMAGE_PATH = (
    OUTPUT_DIR / "svm_rbf_final_test_confusion_matrix.png"
)
FINAL_TEST_NORMALIZED_CONFUSION_MATRIX_IMAGE_PATH = (
    OUTPUT_DIR / "svm_rbf_final_test_confusion_matrix_normalized.png"
)
...
def evaluate_final_test(best_params, X_train_pool, y_train_pool, X_final_test, final_test):
    ...
    model = base.build_svm_rbf_pipeline()
...
def build_summary_log(
    ...
    """生成 SVM-RBF V2 调参日志，明确区分内部调参与外部测试。"""
...
def main():
    """执行 SVM-RBF V2：train_pool 内部调参 + final_test 外部测试。"""
    ...
    base.plot_param_heatmap(results_df, PROJECT_ROOT / PARAM_HEATMAP_IMAGE_PATH)
    ...
    save_confusion_matrix_image(
        ...
        "SVM-RBF V2 Final Test Confusion Matrix",
    )
    save_confusion_matrix_image(
        final_test_predictions["y_true"],
        final_test_predictions["y_pred"],
        ...
        "SVM-RBF V2 Final Test Normalized Confusion Matrix",
        normalized=True,
    )
    ...
    print("SVM-RBF GroupKFold V2 调参实验完成。输出文件如下：")
