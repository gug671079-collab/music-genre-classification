...
import tune_random_forest_groupkfold as base
...
OUTPUT_DIR = Path("logs") / "random_forest_tuning_v2"
...
OOF_CONFUSION_MATRIX_IMAGE_PATH = OUTPUT_DIR / "random_forest_confusion_matrix.png"
...
FINAL_TEST_PREDICTIONS_PATH = OUTPUT_DIR / "random_forest_final_test_predictions.csv"
FINAL_TEST_CONFUSION_MATRIX_IMAGE_PATH = (
    OUTPUT_DIR / "random_forest_final_test_confusion_matrix.png"
)
FINAL_TEST_NORMALIZED_CONFUSION_MATRIX_IMAGE_PATH = (
    OUTPUT_DIR / "random_forest_final_test_confusion_matrix_normalized.png"
)
...
def get_param_grid():
    """默认使用完整参数网格；FAST_MODE 仅用于快速检查代码能否跑通。"""
    if FAST_MODE:
        return {
    ...
    return base.get_param_grid()


def run_grid_search(X, y, groups):
    """在 train_pool 内部执行 RandomForest GridSearchCV，并显示进度。"""
    param_grid = get_param_grid()
    param_count = len(list(ParameterGrid(param_grid)))
    total_fit_count = param_count * N_SPLITS

    print("开始 RandomForest GridSearchCV 调参……", flush=True)
    print(f"参数组合数量：{param_count}", flush=True)
    print(f"交叉验证折数：{N_SPLITS}", flush=True)
    print(f"预计训练次数：{total_fit_count}", flush=True)
    ...
    grid_search = GridSearchCV(
        estimator=base.build_random_forest_pipeline(),
    ...
    print(f"RandomForest GridSearchCV 总耗时：{elapsed:.2f} 秒", flush=True)
    print(f"最佳参数：{grid_search.best_params_}", flush=True)
    print(f"最佳 mean_test_f1_macro：{grid_search.best_score_:.4f}", flush=True)
...
def evaluate_final_test(best_params, X_train_pool, y_train_pool, X_final_test, final_test):
    ...
    model = base.build_random_forest_pipeline()
...
def build_summary_log(
    ...
    """生成 RandomForest V2 调参日志，明确区分内部调参与外部测试。"""
    ...
    return "\n".join(
        [
            "RandomForest GroupKFold 调参实验 V2",
            "=" * 50,
            "",
            "一、实验设置",
...
def main():
    """执行 RandomForest V2：train_pool 内部调参 + final_test 外部测试。"""
    ...
    grid_search = run_grid_search(X_train_pool, y_train_pool, groups_train_pool)
    ...
    base.plot_param_effects(results_df, PROJECT_ROOT / PARAM_EFFECTS_IMAGE_PATH)
    ...
    save_confusion_matrix_image(
        ...
        "RandomForest V2 Final Test Confusion Matrix",
    )
    save_confusion_matrix_image(
        final_test_predictions["y_true"],
        final_test_predictions["y_pred"],
        ...
        "RandomForest V2 Final Test Normalized Confusion Matrix",
        normalized=True,
    )
    ...
    print("RandomForest GroupKFold V2 调参实验完成。输出文件如下：", flush=True)
    for output_file in output_files:
        print(output_file.as_posix(), flush=True)
