...
OUTPUT_DIR = Path("logs") / "random_forest_tuning"
RESULTS_CSV_PATH = OUTPUT_DIR / "random_forest_tuning_results.csv"
SUMMARY_PATH = OUTPUT_DIR / "random_forest_tuning_summary.txt"
TOP_PARAMS_IMAGE_PATH = OUTPUT_DIR / "random_forest_top_params.png"
PARAM_EFFECTS_IMAGE_PATH = OUTPUT_DIR / "random_forest_param_effects.png"
CONFUSION_MATRIX_IMAGE_PATH = OUTPUT_DIR / "random_forest_confusion_matrix.png"
NORMALIZED_CONFUSION_MATRIX_IMAGE_PATH = (
    OUTPUT_DIR / "random_forest_confusion_matrix_normalized.png"
)
PER_CLASS_METRICS_IMAGE_PATH = OUTPUT_DIR / "random_forest_per_class_metrics.png"
FOLD_SCORES_CSV_PATH = OUTPUT_DIR / "random_forest_fold_scores.csv"
FOLD_SCORES_IMAGE_PATH = OUTPUT_DIR / "random_forest_fold_scores.png"
OOF_PREDICTIONS_PATH = OUTPUT_DIR / "random_forest_oof_predictions.csv"
...
def require_matplotlib():
    ...
    try:
        ...
        raise ImportError(
            "缺少 matplotlib，无法生成 RandomForest 调参图像。请先安装 matplotlib，"
...
def build_random_forest_pipeline():
    """构建 RandomForest 调参使用的 Pipeline，保持与 KNN/SVM 相同流程。"""
    return Pipeline(
        steps=[
...
def run_grid_search(X, y, groups):
    """使用 GroupKFold 和 GridSearchCV 搜索 RandomForest 最优参数。"""
    scoring = {
        "accuracy": "accuracy",
...
def plot_top_params(top_10_results, output_path):
    """绘制排名前 10 的 RandomForest 参数组合 Macro-F1 横向柱状图。"""
    plt = require_matplotlib()
    plot_df = top_10_results.sort_values("mean_test_f1_macro", ascending=True)
    labels = [
        f"trees={row.param_model__n_estimators}, "
        f"depth={row.param_model__max_depth}, "
        f"feat={row.param_model__max_features}, "
        f"split={row.param_model__min_samples_split}, "
        f"leaf={row.param_model__min_samples_leaf}"
        for row in plot_df.itertuples(index=False)
    ]

    figure, axis = plt.subplots(figsize=(13, 8))
    axis.barh(labels, plot_df["mean_test_f1_macro"])
    axis.set_title("Top 10 RandomForest Parameter Combinations by GroupKFold Macro F1")
...
def plot_param_effects(results_df, output_path):
    """绘制 RandomForest 关键参数对 Macro-F1 的一维平均影响。"""
    plt = require_matplotlib()
    param_columns = [
    ...
    for axis, (column, title) in zip(axes, param_columns):
        effect_df = (
            results_df.assign(param_label=results_df[column].astype(str))
            .groupby("param_label", sort=False)["mean_test_f1_macro"]
            .mean()
            .reset_index()
        )
        axis.plot(
            effect_df["param_label"],
            effect_df["mean_test_f1_macro"],
            marker="o",
        )
    ...
    axes[-1].axis("off")
    figure.suptitle("RandomForest Parameter Effects on GroupKFold Macro F1", y=1.02)
    figure.tight_layout()
    figure.savefig(output_path, dpi=200, bbox_inches="tight")
    plt.close(figure)
...
def plot_confusion_matrix(y_true, y_pred, labels, output_path):
    ...
    axis.set_title("RandomForest GroupKFold OOF Confusion Matrix")
...
def plot_normalized_confusion_matrix(y_true, y_pred, labels, output_path):
    ...
    axis.set_title("RandomForest GroupKFold OOF Normalized Confusion Matrix")
    axis.set_xlabel("Predicted Label")
    axis.set_ylabel("True Label")
    axis.set_xticks(range(len(labels)))
    axis.set_yticks(range(len(labels)))
...
def plot_per_class_metrics(per_class_metrics, output_path):
    ...
    axis.set_title("RandomForest GroupKFold OOF Per-class Metrics")
...
def plot_fold_scores(fold_scores, output_path):
    ...
    axis.set_title("RandomForest Best Params GroupKFold Fold Macro F1")
...
def build_summary_log(
    ...
    """生成 RandomForest GroupKFold 调参实验总结日志。"""
    ...
    summary_lines = [
        "RandomForest GroupKFold 调参实验",
        "=" * 50,
        "",
        "一、实验目的",
        "在与 KNN、SVM-RBF 调参相同的数据处理、分组方式、交叉验证方式和评价指标下，对 RandomForest 进行参数搜索。",
        ...
        "",
        "六、模型与验证方式",
        "使用的模型：RandomForestClassifier",
        f"验证方式：GroupKFold(n_splits={N_SPLITS})",
        "主要评价指标：Macro-F1",
        "RandomForestClassifier 设置 random_state=42；GridSearchCV 使用 n_jobs=-1，模型内部不额外设置 n_jobs，避免嵌套并行。",
        "",
        "七、参数搜索范围",
        str(param_grid),
        "n_estimators 控制森林中决策树数量；max_depth 控制单棵树最大深度；max_features 控制每次分裂时考虑的特征数量；min_samples_split 控制内部节点继续分裂所需的最小样本数；min_samples_leaf 控制叶节点所需的最小样本数。",
        "",
        "八、最优参数与分数",
        f"最优参数：{best_params}",
        ...
        "",
        "十四、与 Baseline V3、KNN 调参和 SVM-RBF 调参的比较说明",
        "本实验与 Baseline V3、KNN 调参、SVM-RBF 调参实验保持相同的数据输入、字段处理方式、Pipeline 结构、GroupKFold 分组方式和主要评价指标，因此可以用于后续模型横向比较。Baseline V3 是固定参数 KNN，KNN 调参、SVM-RBF 调参和本实验分别在相同评估框架下搜索各自模型的关键参数。",
        "",
        "十五、输出文件列表",
        RESULTS_CSV_PATH.as_posix(),
        SUMMARY_PATH.as_posix(),
        TOP_PARAMS_IMAGE_PATH.as_posix(),
        PARAM_EFFECTS_IMAGE_PATH.as_posix(),
...
def main():
    """执行 RandomForest GroupKFold 调参实验，并保存结果、图像和日志。"""
    ...
    plot_param_effects(results_df, PROJECT_ROOT / PARAM_EFFECTS_IMAGE_PATH)
    ...
    print("RandomForest GroupKFold 调参实验完成。输出文件如下：")
